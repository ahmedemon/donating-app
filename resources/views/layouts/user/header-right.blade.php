...
<style>
    ...
    .timeline li:hover {
        transition-duration: 0.3s;
        background-color: #6e6e6e41 !important;
    }
...
<ul class="navbar-nav header-right">
    <li class="nav-item dropdown notification_dropdown">
        <a class="nav-link" href="javascript:void(0);" role="button" data-bs-toggle="dropdown">
            ...
            @php
                $count_notification = \App\Models\PurchasedProduct::where('owner_id', Auth::user()->id)
                    ->where('seen', 0)
                    ->count();
            @endphp
            <span class="badge light text-white bg-primary rounded-circle">{{ $count_notification }}</span>
        </a>
        <div class="dropdown-menu dropdown-menu-end border-light" style="border: 1px solid !important;">
            <div id="DZ_W_Notification1" class="widget-media dlab-scroll p-3" style="height:380px;">
                <ul class="timeline">
                    @include('layouts.user.notification')
                </ul>
            </div>
            <a class="all-notification" href="{{ route('buyer-request.all.notifications.request') }}">See all notifications <i class="ti-arrow-end"></i></a>
        </div>
    </li>

...
class BuyerRequestController extends Controller
{
    ...
    public function pending()
    {
        ...
        if (request()->ajax()) {
            $data = PurchasedProduct::where('owner_id', $user_id)->where('status', 0)->where('owner_approval', 0)->with('donation')->latest()->get();
    ...
    public function completed()
    {
        ...
        if (request()->ajax()) {
            $data = PurchasedProduct::where('owner_id', $user_id)->where('status', 1)->where('owner_approval', 1)->with('donation')->latest()->get();
            return DataTables::of($data)
                ->addIndexColumn()
                ->addColumn('product', function($data){
                    $title = 'Title: ' . $data->donation->title ?? '<span class="badge badge-danger">Not Found</span>';
    ...
    public function rejected()
    {
        ...
        if (request()->ajax()) {
            $data = PurchasedProduct::where('owner_id', $user_id)->where('status', 2)->where('owner_approval', 2)->with('donation')->latest()->get();
            return DataTables::of($data)
                ->addIndexColumn()
                ->addColumn('product', function($data){
                    $title = 'Title: ' . $data->donation->title ?? '<span class="badge badge-danger">Not Found</span>';
